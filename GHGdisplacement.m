%inputs  % only for FG as internal heat generator   
Feed=1.618;    
FPW =0.177;
Fuelgas=0.2595;
TE=926.632;
z = 1e6/18535;
[energy_allocation] = 1; %en_all_fuel_TE(Fuelgas,TE);
No=0;
Efficiency=.34;
charamount=.14; 
Naphthaamount=.21;
...
crN2O= FGe.*(1.418/1e6)+Che .* (0.078.* 1.026+0.098)/1e6+ Ne.* ( 0.078.*0.99 +0.138)/1e6 ; 
...
CO2_wofull = (CO2c+CO2p+iCO2+CO2nc+crCO2).*z + tCO2;
N2O_full= (N20c+iN20+N20nc+N20p+crN2O).*z+ tN2O;
CH4_full= (CH4c+iCH4+CH4nc+CH4p+crCH4).*z+ tCH4;
CO_full= (COc+iCO+COnc+COp+crCO).*z+ tCO;
VOC_full= (VOCc+iVOC+VOCnc+VOCp+crVOC).*z+ tVOC;
...
GHG = TGHG+ CO2_full + 3.148* VOC_full + 1.592* CO_full+ 30* CH4_full + 265* N2O_full;% + 900 * BC_full - 69* OC_full - 11 * NOx_full;
